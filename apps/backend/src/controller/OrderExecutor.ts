...
export class OrderExecutor {
  ...
  async executeDecisions(
    ...
    if (this.mode === "paper") {
      logger.info(
        "📝 PAPER MODE: Executing orders in virtual account (모의투자) via KIS API"
      );
    ...
    for (const decision of decisions) {
      ...
      try {
        ...
        await this.db.saveOrder(runId, null, result, decision.name);
      } catch (error) {
        ...
        await this.db.saveOrder(runId, null, failedResult, decision.name);
      }
  ...
  private async executeBuyOrder(
    ...
    try {
      // ⚠️ BUY orders MUST specify quantity (number of shares)
      let quantity = decision.quantity || 0;
      let currentPrice = 0;

      // ⚠️ Fallback: Calculate quantity from amount_krw (for backward compatibility)
      // This should NOT be used in production - AI should always provide quantity
      if (quantity === 0 && decision.amount_krw) {
        logger.warn(
          "⚠️ BUY order without quantity - calculating from amount_krw (this should not happen)",
          {
            ticker: decision.ticker,
        ...
        if (market === Market.DOMESTIC) {
          ...
          if (priceData.rt_cd !== "0") {
            throw new TradingError(
              `Failed to get overseas price: ${
                priceData.msg1 || priceData.msg_cd
              }`
            );
          }
          ...
          if (Array.isArray(output)) {
            currentPrice = parseFloat(
              output[0]?.last || output[0]?.LAST || output[0]?.xymd_cls_prc || 0
            );
          } else {
            currentPrice = parseFloat(
        ...
        logger.info("Calculated quantity from amount_krw", {
          ticker: decision.ticker,
          amount_krw: decision.amount_krw,
      ...
      // ⚠️ Minimum 1 share per order
      if (quantity < 1) {
        throw new TradingError(
          `BUY order quantity must be at least 1 share (got ${quantity})`
        );
      }
      ...
      logger.info("📤 Placing market buy order", {
        ...
        mode: this.mode,
        account_type:
          this.mode === "paper" ? "Virtual (모의투자)" : "Real (실거래)",
      });

      // Place market order based on market type
      let orderResponse;
      if (market === Market.DOMESTIC) {
        // Domestic stock - use domestic order API (01 = 시장가/market order)
        orderResponse = await retryWithBackoff(() =>
          this.kisApi.buyOrder(
            this.accountNumber,
            decision.ticker,
            quantity,
            0,
      ...
      // rt_cd: "0" = success, "1" = failure
      if (orderResponse.rt_cd !== "0") {
        const errorMsg =
          orderResponse.msg1 ||
          orderResponse.msg_cd ||
          "Unknown error from broker";
        logger.error("❌ Order rejected by broker", {
          ticker: decision.ticker,
          rt_cd: orderResponse.rt_cd,
          msg_cd: orderResponse.msg_cd,
          msg1: orderResponse.msg1,
        });
        throw new TradingError(`Order rejected: ${errorMsg}`);
      }

      const orderNumber =
        orderResponse.output?.ODNO || orderResponse.output?.odno || "";
      ...
      const fillDetails = await this.getOrderFillDetails(
        orderNumber,
        today,
        decision.ticker,
        market
  ...
  private async executePaperOrder(
    ...
    logger.warn(
      "⚠️ PAPER MODE: This is a SIMULATION - No real trades will be executed!",
      {
        ticker: decision.ticker,
        action: decision.action,
        mode: this.mode,
    ...
    if (decision.action === "BUY") {
      ...
      if (quantity === 0 && decision.amount_krw) {
        logger.warn(
          "⚠️ PAPER BUY order without quantity - calculating from amount_krw",
          {
            ticker: decision.ticker,
            amount_krw: decision.amount_krw,
          }
        ...
        quantity = Math.floor(decision.amount_krw / currentPrice);
        logger.info("Calculated quantity from amount_krw (paper mode)", {
          ticker: decision.ticker,
          amount_krw: decision.amount_krw,
          current_price: currentPrice,
          calculated_quantity: quantity,
      ...
      if (quantity < 1) {
        throw new TradingError(
          `PAPER BUY order quantity must be at least 1 share (got ${quantity})`
        );
      }
   ...
   */
  private async getOrderFillDetails(
    orderNumber: string,
    date: string,
    ticker: string,
    market: Market
    ...
    try {
      ...
      if (market === Market.DOMESTIC) {
        ...
        const exchangeCode = this.getExchangeCode(market);
        history = await this.kisApi.getOverseasOrderHistory(
          this.accountNumber,
          exchangeCode,
          date,
          date
      ...
      const order = history.output.find(
        (o: any) =>
          (o.odno === orderNumber ||
            o.ODNO === orderNumber ||
            o.ord_no === orderNumber ||
            o.ORD_NO === orderNumber) &&
          (o.pdno === ticker ||
            o.PDNO === ticker ||
            o.symbl === ticker ||
      ...
      if (!order) {
        logger.warn("Order not found in history", {
          orderNumber,
          ticker,
          date,
          market,
      ...
      logger.warn("Failed to get order fill details", {
        orderNumber,
        ticker,
        market,
        error,
