...
const US_STOCKS = [
  ...
  },
  {
    ...
    nameEn: "The Procter & Gamble Company",
  },
  {
    ticker: "UNH",
  ...
  },
  {
    ...
    nameEn: "The Home Depot, Inc.",
  },
  {
    ticker: "DIS",
    name: "The Walt Disney Company",
    nameEn: "The Walt Disney Company",
  },
  {
    ticker: "BAC",
  ...
  { ticker: "INTC", name: "Intel Corporation", nameEn: "Intel Corporation" },
  {
    ...
    nameEn: "Comcast Corporation",
  },
  { ticker: "PEP", name: "PepsiCo, Inc.", nameEn: "PepsiCo, Inc." },
  {
    ticker: "COST",
    name: "Costco Wholesale Corporation",
    nameEn: "Costco Wholesale Corporation",
  },
  {
    ticker: "TMO",
    name: "Thermo Fisher Scientific Inc.",
    nameEn: "Thermo Fisher Scientific Inc.",
  },
  ...
  { ticker: "DHR", name: "Danaher Corporation", nameEn: "Danaher Corporation" },
  {
    ...
    nameEn: "Verizon Communications Inc.",
  },
  { ticker: "LIN", name: "Linde plc", nameEn: "Linde plc" },
  {
    ticker: "PM",
    name: "Philip Morris International Inc.",
    nameEn: "Philip Morris International Inc.",
  },
  {
    ticker: "NEE",
    name: "NextEra Energy, Inc.",
    nameEn: "NextEra Energy, Inc.",
  },
  {
    ticker: "QCOM",
  ...
  { ticker: "RTX", name: "RTX Corporation", nameEn: "RTX Corporation" },
  {
    ...
    nameEn: "Honeywell International Inc.",
  },
  ...
  },
  {
    ...
    nameEn: "Lowe's Companies, Inc.",
  },
  { ticker: "INTU", name: "Intuit Inc.", nameEn: "Intuit Inc." },
  {
    ticker: "BKNG",
    name: "Booking Holdings Inc.",
    nameEn: "Booking Holdings Inc.",
  },
  {
    ticker: "SBUX",
  ...
  },
  {
    ...
    nameEn: "Gilead Sciences, Inc.",
  },
  {
    ticker: "ADP",
    name: "Automatic Data Processing, Inc.",
    nameEn: "Automatic Data Processing, Inc.",
  },
  {
    ticker: "ISRG",
    name: "Intuitive Surgical, Inc.",
    nameEn: "Intuitive Surgical, Inc.",
  },
  {
    ticker: "GE",
  ...
  { ticker: "MDT", name: "Medtronic plc", nameEn: "Medtronic plc" },
  {
    ...
    nameEn: "American Tower Corporation",
  },
  ...
  { ticker: "EQIX", name: "Equinix, Inc.", nameEn: "Equinix, Inc." },
  {
    ...
    nameEn: "Regeneron Pharmaceuticals, Inc.",
  },
  {
    ticker: "CDNS",
  ...
  },
  {
    ...
    nameEn: "Zoom Video Communications, Inc.",
  },
  {
    ticker: "DOCN",
    name: "DigitalOcean Holdings, Inc.",
    nameEn: "DigitalOcean Holdings, Inc.",
  },
  ...
  },
  {
    ...
    nameEn: "Robinhood Markets, Inc.",
  },
  {
    ticker: "SOFI",
    name: "SoFi Technologies, Inc.",
    nameEn: "SoFi Technologies, Inc.",
  },
  {
    ticker: "RIVN",
    name: "Rivian Automotive, Inc.",
    nameEn: "Rivian Automotive, Inc.",
  },
...
export const searchStocks = async (req: Request, res: Response) => {
  try {
    ...
    const marketParam = (
      (req.query.market as string) || Market.DOMESTIC
    ).toUpperCase();
    // Convert string to Market enum
    const market =
      Object.values(Market).find((m) => m === marketParam) || Market.DOMESTIC;
    ...
    // For Korean market, use master file parser
    if (market === Market.DOMESTIC) {
      // Check if query looks like a ticker (6-digit number)
      const isTickerLike = /^\d{6}$/.test(query);

      // Search in master files
      const masterResults = stockMasterParser.search(query, "DOMESTIC");

      // If ticker search, try to enrich with real-time data from KIS API
      if (isTickerLike && masterResults.length > 0) {
        const tickerResult = masterResults.find((r) => r.ticker === query);
        if (tickerResult) {
          try {
            await kisRateLimiter.waitIfNeeded();
            const priceData = await getKISApi().getCurrentPrice(query);
            const output = priceData.output || {};

            if (output.stck_prpr) {
              // Found valid stock via KIS API, use real-time name
              return res.json({
                success: true,
                data: [
                  {
                    ticker: query,
                    name: output.hts_kor_isnm || tickerResult.name,
                    nameEn: "",
                    market: "DOMESTIC",
            ...
            logger.warn(
              "KIS API search failed for ticker, using master file data",
              { error, query }
            );
          }
        }
      ...
      // Return results from master file
      return res.json({
        success: true,
        data: masterResults.slice(0, 20).map((stock) => ({
          ticker: stock.ticker,
          name: stock.name,
          nameEn: stock.nameEn,
          market: "DOMESTIC",
    ...
    // For US market, use master file parser
    if (market === Market.US) {
      ...
      // Search in master files
      const masterResults = stockMasterParser.search(query, "US");

      if (isTickerLike && masterResults.length > 0) {
        const tickerResult = masterResults.find(
          (r) => r.ticker.toUpperCase() === query.toUpperCase()
        );
        if (tickerResult) {
          try {
            await kisRateLimiter.waitIfNeeded();
            const kisResult = await getKISApi().searchOverseasStock(
              query.toUpperCase(),
              "NAS"
            );
                ...
                data: [
                  {
                    ...
                    name: kisResult.name || tickerResult.name,
                    nameEn: kisResult.nameEn || tickerResult.nameEn,
                    market: "US",
            ...
            logger.warn("KIS API search failed, using master file data", {
              error,
              query,
            });
          }
...
export const getPopularStocks = async (req: Request, res: Response) => {
  try {
    ...
    if (market === Market.DOMESTIC) {
      ...
      const stocks = KOREAN_STOCKS.slice(
        0,
        Math.min(limit, KOREAN_STOCKS.length)
      ).map((stock) => ({
        ticker: stock.ticker,
        name: stock.name,
        nameEn: stock.nameEn,
    ...
    if (market === Market.US) {
      ...
      try {
        ...
        logger.warn(
          "KIS API popular stocks failed, falling back to local database",
          { error }
        );
      }

      // Fallback to local database
      const stocks = US_STOCKS.slice(0, Math.min(limit, US_STOCKS.length)).map(
        (stock) => ({
          ticker: stock.ticker,
          name: stock.name,
          nameEn: stock.nameEn,
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to get popular stocks",
    });
  }
};
...
export const getStockDetail = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to get stock detail",
    });
  }
};
...
export const getStockHistory = async (req: Request, res: Response) => {
  try {
    const { ticker } = req.params;
    const marketParam = req.query.market?.toString().toUpperCase();
    ...
    logger.info("Get stock history request", { ticker, market, days });

    const dataCollector = new DataCollector();
    const historyData = await dataCollector.getStockHistory(
      ticker,
      days,
      market
    );
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to get stock history",
    });
  }
};
