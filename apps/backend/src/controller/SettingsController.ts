...
export const getRecentOrders = async (req: Request, res: Response) => {
    try {
        ...
        const { data, error } = await getSettingsRepo()['db']
            .from('orders')
            .select('id, run_id, ticker, name, direction, order_type, requested_qty, requested_price, filled_qty, avg_filled_price, status, broker_order_id, error_code, error_message, created_at, updated_at, runs(id, started_at, status, market)')
