...
export class KISApiClient {
  ...
  async buyOrder(
    ...
    quantity: number,
    price: number = 0,
    orderType: string = "01" // 00=지정가(limit), 01=시장가(market)
  ): Promise<any> {
    const mode = process.env.MODE || "paper";
    const trId = mode === "live" ? "TTTC0802U" : "VTTC0802U";
    const cleanAccountNumber = accountNumber.replace(/-/g, "");
  ...
  async sellOrder(
    ...
    const mode = process.env.MODE || "paper";
    const trId = mode === "live" ? "TTTC0801U" : "VTTC0801U";
    const cleanAccountNumber = accountNumber.replace(/-/g, "");
  ...
  async cancelOrder(
    ...
    const mode = process.env.MODE || "paper";
    const trId = mode === "live" ? "TTTC0803U" : "VTTC0803U";
    const cleanAccountNumber = accountNumber.replace(/-/g, "");
  ...
  async getOrderHistory(accountNumber: string, date: string): Promise<any> {
    ...
    const trId = mode === "live" ? "TTTC8001R" : "VTTC8001R";
   ...
   * 해외주식 잔고조회
   */
  async getOverseasAccountBalance(
    accountNumber: string,
   ...
   * 해외속보(제목)
   *
   * @param newsProviderCode - News provider code (0: All)
   * @param screenDivCode - Screen division code (11801)
   * @param marketClassCode - Market classification code (optional)
   * @param ticker - Stock ticker (optional)
   * @param titleContent - Title content search (optional)
   * @param inputDate - Input date (YYYYMMDD) (optional)
   * @param inputHour - Input hour (HHMMSS) (optional)
   * @param rankSortCode - Rank sort code (optional)
   * @param serialNumber - Serial number (optional)
   * @returns Breaking news titles (max 100 items)
   */
  ...
  /**
   * Get overseas stock price detail with pagination support
   * 해외주식 현재가상세 (페이지네이션 지원)
   *
   * @param ticker - Stock ticker symbol
   * @param exchangeCode - Exchange code (HKS, NYS, NAS, AMS, TSE, SHS, SZS, SHI, SZI, HSX, HNX, BAY, BAQ, BAA)
   * @param maxDepth - Maximum pagination depth (default: 10)
  ...
  async getOverseasPriceDetail(
    ...
    const fetchPage = async (
      ...
      if (depth >= maxDepth) {
        logger.warn("Maximum pagination depth reached", { maxDepth, ticker });
        return;
      }
      ...
      if (nextTrCont === "M") {
        logger.info("Fetching next page for price detail", {
          ticker,
          depth: depth + 1,
        });
  ...
  /**
   * Get overseas stock daily prices with pagination support
   * 해외주식 기간별시세 (페이지네이션 지원)
   *
   * @param ticker - Stock ticker symbol
   * @param exchangeCode - Exchange code (HKS, NYS, NAS, AMS, TSE, SHS, SZS, SHI, SZI, HSX, HNX)
   * @param period - Period type ("0"=Daily, "1"=Weekly, "2"=Monthly)
   * @param baseDate - Base date (YYYYMMDD), empty string for today
   * @param adjustPrice - Adjust for stock splits ("0"=No, "1"=Yes)
   * @param maxDepth - Maximum pagination depth (default: 10)
   * @returns Combined data from all pages
   */
  async getOverseasDailyPrices(
    ...
    let firstResponseStatus = { rt_cd: "0", msg_cd: "", msg1: "" };
    ...
    const fetchPage = async (
      ...
      // TR ID는 실전/모의투자 모두 동일
      const trId = "HHDFS76240000";

      const formattedDate = baseDate ? baseDate.replace(/-/g, "") : "";
      ...
      // ⚠️ Capture first response status for error reporting
      if (depth === 0) {
        firstResponseStatus = {
          rt_cd: data.rt_cd || "0",
          msg_cd: data.msg_cd || "",
      ...
      if (nextTrCont === "M" || nextTrCont === "F") {
        logger.info("Fetching next page", { ticker, depth: depth + 1 });
        await new Promise((resolve) => setTimeout(resolve, 100));
        await fetchPage("N", depth + 1);
      }
    };
    ...
    return {
      output1: allOutput1,
      output2: allOutput2,
      rt_cd: firstResponseStatus.rt_cd,
      msg_cd: firstResponseStatus.msg_cd,
      msg1: firstResponseStatus.msg1,
    };
  }
