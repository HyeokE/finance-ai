...
export class DatabaseRepository {
  ...
  async createRun(
    ...
    try {
      ...
      if (error)
        throw new DatabaseError("Failed to create run", error.code, error);
  ...
  async updateRunStatus(
    ...
    try {
      ...
      if (error)
        throw new DatabaseError("Failed to update run", error.code, error);
  ...
  async saveDecisions(runId: string, decisions: Decision[]): Promise<void> {
    try {
      ...
      if (error)
        throw new DatabaseError("Failed to save decisions", error.code, error);

      logger.info("Saved AI decisions", {
        run_id: runId,
  ...
  async getDecisions(runId: string): Promise<Decision[]> {
    ...
    if (error)
      throw new DatabaseError("Failed to get decisions", error.code, error);
  ...
  async saveOrder(
    ...
    try {
      ...
      if (error)
        throw new DatabaseError("Failed to save order", error.code, error);
  ...
  async updateOrder(
    ...
    if (error)
      throw new DatabaseError("Failed to update order", error.code, error);
  ...
  async savePortfolioSnapshot(
    ...
    try {
      ...
      logger.error("Failed to save portfolio snapshot", {
        run_id: runId,
        error,
      });
      throw error;
  ...
  /**
   * Get daily PnL trend (one data point per day)
   */
  async getDailyPnLTrend(days: number = 30): Promise<any[]> {
    ...
    if (error)
      throw new DatabaseError("Failed to get PnL trend", error.code, error);

    if (!data) return [];

    // Group by date (YYYY-MM-DD) and keep only the last snapshot per day
    const dailyData = new Map<string, any>();

    for (const snapshot of data) {
      const date = new Date(snapshot.created_at).toISOString().split("T")[0]; // YYYY-MM-DD
      // Always keep the latest snapshot for each date
      // Since data is sorted chronologically, later snapshots will overwrite earlier ones
      dailyData.set(date, snapshot);
    }
  ...
  async getOrderStats(): Promise<any> {
    ...
    if (error)
      throw new DatabaseError("Failed to get order stats", error.code, error);
