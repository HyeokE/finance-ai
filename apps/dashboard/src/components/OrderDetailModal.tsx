        ...
        <div className="px-6 pb-6 space-y-4">
            ...
            <CardContent className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2">
              ...
              <DetailItem
                ...
                value={
                  <Badge
                    variant={
                      order.direction === "buy" ? "secondary" : "outline"
                    }
                  >
          ...
          <Card className="border-border/50 shadow-none">
            <CardContent className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2">
              <DetailItem
                label="요청 수량"
                value={`${formatNumber(order.requested_qty)} 주`}
              />
              <DetailItem
                label="체결 수량"
                value={`${formatNumber(order.filled_qty || 0)} 주`}
              />
              <DetailItem
                label="평균 체결가"
                value={
                  typeof order.avg_filled_price === "number"
              ...
              <DetailItem
                ...
                value={
                  order.filled_qty && order.avg_filled_price
                    ? formatKRW(order.filled_qty * order.avg_filled_price)
                    : "-"
                }
              />
              {typeof order.requested_price === "number" && (
                <DetailItem
                  label="요청 가격"
                  value={formatKRW(order.requested_price)}
                />
              )}
              {order.order_type && (
                <DetailItem label="주문 유형" value={order.order_type} />
              )}
            </CardContent>
          </Card>
            ...
            <Card className="border-border/50 shadow-none">
              <CardContent className="grid grid-cols-1 gap-3 p-4">
                <DetailItem
                  label="증권사 주문 ID"
                  value={
                    <span className="font-mono">{order.broker_order_id}</span>
                  }
                />
              </CardContent>
            </Card>
          ...
          {(order.error_code || order.error_message) && (
            <Card className="border-border/50 shadow-none">
              <CardContent className="grid grid-cols-1 gap-3 p-4">
                ...
                {order.error_code && (
                  <DetailItem label="에러 코드" value={order.error_code} />
                )}
                {order.error_message && (
                  <DetailItem label="에러 메시지" value={order.error_message} />
                )}
              </CardContent>
            </Card>
          )}
          ...
          <Card className="border-border/50 shadow-none">
            <CardContent className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2">
              ...
              {order.runs?.started_at && (
                <DetailItem
                  label="배치 시작 시각"
                  value={formatDate(order.runs.started_at)}
                />
              )}
              {order.runs?.status && (
                <DetailItem label="배치 상태" value={order.runs.status} />
              )}
            </CardContent>
          </Card>
