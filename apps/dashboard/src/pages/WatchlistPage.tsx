...
import { stocksApi, watchlistApi, type StockSearchResult } from "../api/client";
...
export function WatchlistPage({ isActive }: Props) {
  ...
  const popularStocksQuery = usePopularStocksQuery(
    selectedMarket,
    showAddModal && !searchQuery
  );

  // Fetch detailed watchlist data
  const watchlistDetailedQuery = useQuery({
    queryKey: ["watchlist-detailed", selectedMarket],
    queryFn: () => watchlistApi.getDetailedByMarket(selectedMarket),
    enabled: isActive,
  ...
  const watchlist = watchlistDetailedQuery.data || watchlistQuery.data || [];
  const popularStocks = popularStocksQuery.data || [];
  const isLoadingDetailed = watchlistDetailedQuery.isLoading;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <Select
            ...
            className="sm:w-44 bg-secondary/50 border-border/50"
          >
            ...
            <>
              ...
              <div className="space-y-3">
                {watchlist.map((item, index) => (
                  <div
                    key={item.id}
                    className="glass animate-slide-up rounded-lg border border-border/50 p-4 transition-all hover:border-primary/30 hover:bg-secondary/30"
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      >
                        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10 text-2xl ring-1 ring-primary/20">
                          {
                            MARKET_INFO[item.market as keyof typeof MARKET_INFO]
                              ?.flag
                          }
                        </div>
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleToggleStock(item.id, item.enabled)
                          }
                        >
                          {item.enabled ? "끄기" : "켜기"}
                        </Button>
                    ...
                    {/* Detailed Data */}
                    {isLoadingDetailed && !item.detailed_data && (
                    ...
                    {!isLoadingDetailed && !item.detailed_data && (
                      <div className="flex items-center justify-center py-4 text-sm text-yellow-500/80">
                        ⚠️ 상세 데이터를 불러올 수 없습니다
                      </div>
                    )}
                    ...
                    {item.detailed_data && (
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-3 border-t border-border/30">
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="text-lg font-bold text-foreground terminal-number">
                            {item.detailed_data.current_price.toLocaleString()}
                          </div>
                          <div
                            className={`text-xs font-semibold ${
                              item.detailed_data.from_prev_close_return >= 0
                                ? "text-green-400"
                                : "text-red-400"
                            }`}
                          >
                            {item.detailed_data.from_prev_close_return >= 0
                              ...
                              : ""}
                            {(
                              item.detailed_data.from_prev_close_return * 100
                            ).toFixed(2)}
                            %
                          </div>
                        </div>
                          ...
                          <div className="text-xs space-y-0.5">
                            <div className="flex justify-between">
                              ...
                              <span className="terminal-number">
                                {item.detailed_data.open.toLocaleString()}
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">
                              ...
                              <span className="terminal-number text-green-400">
                                {item.detailed_data.high.toLocaleString()}
                              </span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">
                              ...
                              <span className="terminal-number text-red-400">
                                {item.detailed_data.low.toLocaleString()}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          {item.detailed_data.history_length === 0 ? (
                            <div className="text-xs text-yellow-500/80 py-2">
                              히스토리 데이터 없음
                            </div>
                          ) : (
                            <div className="text-xs space-y-0.5">
                              <div className="flex justify-between">
                                ...
                                <span className="terminal-number">
                                  {item.detailed_data.today_volume > 0
                                    ? `${(
                                        item.detailed_data.today_volume /
                                        1000000
                                      ).toFixed(2)}M`
                                    : "N/A"}
                                </span>
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="terminal-number">
                                  {item.detailed_data.average_volume_30d > 0
                                    ? `${(
                                        item.detailed_data.average_volume_30d /
                                        1000000
                                      ).toFixed(2)}M`
                                    : "N/A"}
                                </span>
                              ...
                              <div className="flex justify-between">
                                ...
                                <span
                                  className={`terminal-number font-semibold ${
                                    item.detailed_data.volume_ratio >= 1
                                      ? "text-green-400"
                                      : item.detailed_data.volume_ratio > 0
                                      ? "text-yellow-400"
                                      : "text-muted-foreground"
                                  }`}
                                >
                                  {item.detailed_data.volume_ratio > 0
                                    ? `${item.detailed_data.volume_ratio.toFixed(
                                        2
                                      )}x`
                                    : "N/A"}
                                </span>
                          ...
                          <div className="text-xs text-muted-foreground">
                            ...
                            {item.detailed_data.history_length > 0 && (
                              <span className="ml-1 text-muted-foreground/60">
                                ({item.detailed_data.history_length}일)
                              </span>
                            )}
                          </div>
                          {item.detailed_data.history_length === 0 ? (
                            ...
                            <div className="text-xs space-y-0.5">
                              <div className="flex justify-between">
                                ...
                                <span
                                  className={`terminal-number ${
                                    item.detailed_data.ema5_position > 1.01
                                      ? "text-green-400"
                                      : item.detailed_data.ema5_position < 0.99
                                      ? "text-red-400"
                                      : "text-muted-foreground"
                                  }`}
                                >
                                  {item.detailed_data.ema5_position.toFixed(3)}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">
                                ...
                                <span
                                  className={`terminal-number ${
                                    item.detailed_data.ema20_position > 1.01
                                      ? "text-green-400"
                                      : item.detailed_data.ema20_position < 0.99
                                      ? "text-red-400"
                                      : "text-muted-foreground"
                                  }`}
                                >
                                  {item.detailed_data.ema20_position.toFixed(3)}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">
                                ...
                                <span className="terminal-number">
                                  {item.detailed_data.volatility_20d > 0
                                    ? `${(
                                        item.detailed_data.volatility_20d * 100
                                      ).toFixed(2)}%`
                                    : "N/A"}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                ...
                                <span className="terminal-number">
                                  {item.detailed_data.atr_14d > 0
                                    ? item.detailed_data.atr_14d.toFixed(2)
                                    : "N/A"}
                                </span>
                              </div>
                            </div>
          ...
          <div className="space-y-4 px-6 pb-6">
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button
                ...
                disabled={
                  !newStock.ticker || !newStock.name || addMutation.isPending
                }
              >
