  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <h2 className="text-2xl font-bold text-foreground lg:text-3xl">주문/체결 기록</h2>
        <p className="text-sm text-muted-foreground">모든 주문 내역과 체결 상태를 확인합니다</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        <Card className="glass border-border/50">
          <CardContent className="p-4">
            ...
            <div className="terminal-number text-2xl font-bold text-foreground">{orders.length}</div>
          </CardContent>
        </Card>
        <Card className="glass border-border/50 glow-success">
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">체결 완료</div>
            <div className="terminal-number text-2xl font-bold text-green-400">{filledOrders}</div>
          </CardContent>
        </Card>
        <Card className="glass border-border/50">
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">대기 중</div>
            <div className="terminal-number text-2xl font-bold text-yellow-400">{pendingOrders}</div>
          </CardContent>
        </Card>
        <Card className="glass border-border/50 glow-destructive">
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">실패</div>
            <div className="terminal-number text-2xl font-bold text-red-400">{failedOrders}</div>
          </CardContent>
        </Card>
        <Card className="glass border-border/50">
          <CardContent className="p-4">
            ...
            <div className="terminal-number text-2xl font-bold text-primary">
              {totalRequested > 0 ? ((totalFilled / totalRequested) * 100).toFixed(0) : 0}%
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border/50 hover:bg-transparent">
                  ...
                  <TableHead className="text-muted-foreground">요청 수량</TableHead>
                  <TableHead className="text-muted-foreground">체결 수량</TableHead>
                  <TableHead className="text-muted-foreground">평균가</TableHead>
              ...
              <TableBody>
                {orders.map((order) => (
                  ...
                  >
                    ...
                    <TableCell className="text-sm text-foreground">{order.runs?.market || 'N/A'}</TableCell>
                    <TableCell className="font-semibold text-foreground">{order.ticker}</TableCell>
                    <TableCell>
                      <Badge variant={order.direction === 'buy' ? 'success' : 'destructive'}>
                        {order.direction.toUpperCase()}
                      </Badge>
                    </TableCell>
                    <TableCell className="terminal-number text-foreground">{order.requested_qty}</TableCell>
                    <TableCell className="terminal-number text-foreground">{order.filled_qty || 0}</TableCell>
